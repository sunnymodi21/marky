...
- **No Accessibility permission, no synthetic keystrokes.** Auto-paste via CGEvent ⌘V was
  implemented and deliberately removed (commit 159a62a). Hotkeys only rewrite the
  clipboard; the user pastes with ⌘V themselves. A frontmost-window screenshot+OCR feature
  was also added and reverted — don't reintroduce Screen Recording permission.
...
│   ├── MenuContentView.swift     # search bar, history list, convert actions, footer
...
|---|---|---|
| `.openHistory` ("openHistory") | ⌥⌘V | toggle the clipboard history panel open/closed |
...
## Keyboard navigation (menu panel)

The panel is fully keyboard-operable (like Windows Win+V):
- **⌥⌘V** (global) toggles the panel open/closed.
- **↑/↓** move selection through history items; Up from the top returns focus to
  the search field.
- **Return** copies the selected entry and closes the panel.
...
./Scripts/package_app.sh release  # SPM binary -> Marky.app (ad-hoc signed, copies *.bundle)
...
changes — a stale running instance keeps the old binary's behavior. Ad-hoc signing in
`package_app.sh` keeps TCC grants sticking across rebuilds.
